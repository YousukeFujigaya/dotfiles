...
if [ ! "$skip_apps" ]; then
    ...
    if is_file "${REPO_DIR}/homebrew/.config/homebrew/Brewfile"; then
        brew bundle install --file "${REPO_DIR}/homebrew/.config/homebrew/Brewfile" --no-lock $([ -n "$verbose" ] && echo -v)
    else
        brew bundle install --file "${GHQ_ROOT_PATH}/github.com/${GITHUB_USER_NAME}/dotfiles/homebrew/.config/homebrew/Brewfile" --no-lock $([ -n "$verbose" ] && echo -v)
    fi
fi
